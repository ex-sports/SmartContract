...
//for extdev classicTestnet
const chainId    = 'extdev-plasma-us1'
const writeUrl   = 'https://extdev-plasma-us1.dappchains.com/rpc'
const readUrl    = 'https://extdev-plasma-us1.dappchains.com/query'
  ...
  networks: {
    loom_mainnet_dev: {
      provider: function () {
        ...
        const mnemonicPath = path.join(__dirname, 'mainnet_dev_mnemonic')
        const privateKeyPath = path.join(__dirname, 'mainnet_dev_private_key')
    ...
    loom_mainnet_service: {
      provider: function () {
        const chainId = 'default'
        const writeUrl = 'http://plasma.dappchains.com/rpc'
        const readUrl = 'http://plasma.dappchains.com/query'
        const mnemonicPath = path.join(__dirname, 'mainnet_service_mnemonic')
        const privateKeyPath = path.join(__dirname, 'mainnet_service_private_key')
        if (fs.existsSync(privateKeyPath)) {
          const loomTruffleProvider = getLoomProviderWithPrivateKey(privateKeyPath, chainId, writeUrl, readUrl)
          return loomTruffleProvider
        } else if (fs.existsSync(mnemonicPath)) {
